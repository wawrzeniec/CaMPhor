"""
camphor.VOI.VOITools

This is the GUI panel for executing VOI extraction algorithms in the CaMPhor GUI
The panel allows the user to choose any of the filters in the VOI/filters directory,
displays configurable parameters and executes the filter on request
"""
...
class VOITools(QtGui.QDockWidget):
    """
    This class implements the VOI extraction tools widget
    """
    def __init__(self, parent):
        super(VOITools,self).__init__(parent=parent)
        self.camphor = parent
    ...
    def iniUI(self):
        ...
        self.filterNames = [os.path.splitext(f)[0] for f in os.listdir('camphor/VOI/filters') if f.endswith('.py')]
        ...
        self.filtermod = [importlib.import_module('camphor.VOI.filters.' + f) for f in self.filterNames]
        ...
        # The "extractVOI" button
        self.extractVOIButton = QtGui.QPushButton('Extract VOIs')
        self.extractVOIButton.clicked.connect(self.doExtraction)

        self.layout = QtGui.QVBoxLayout()
        self.layout.addLayout(self.methodLayout)
        self.layout.addWidget(self.paramFrame)
        self.layout.addStretch(1)
        self.layout.addWidget(self.extractVOIButton)
    ...
    def doExtraction(self):
        ...
        try:
            with VOIExtractionProgress(parent=self) as pdialog:
            ...
            pdialog.setLabelText('Error during VOI extraction!')
            raise
    ...
    def updateEvent(self):
        ...
        print("[{:2g}%]".format(progress.percentDone))
    ...
    def makeParamLayout(self, f):
        ...
        :param mod: python module object for the specified filter (the list of loaded filter modules is in VOITools.filtermod)
        :return:
        """
        ...
        for i, j in enumerate(params):
            if j is not '_paramType' and j is not '_controls':
...
class VOIExtractionProgress(QtGui.QDialog):
    def __init__(self, parent):
        QtGui.QDialog.__init__(self, parent=parent)

        self.parent = parent
        self.layout = QtGui.QVBoxLayout()
        self.label = QtGui.QLabel('Extracting VOIs...')
        ...
        self.setWindowTitle('Executing VOI extraction algorithm ({:s})'.format(self.parent.activeFilterName))

    def cancel(self):
        print('VOI extraction cancelled!')
