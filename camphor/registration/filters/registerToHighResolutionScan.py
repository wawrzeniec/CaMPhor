...
class registerToHighResolutionScan(camphorRegistrationMethod):
    ...
    def registerImage(self, template, data, target):
        ...
        for i, d in enumerate(data):
            ...
            fixed_image = sitk.GetImageFromArray(template[0].astype(numpy.double)) # template is passed as double already so no need to cast
            moving_image = sitk.GetImageFromArray(data[i].astype(numpy.double))
