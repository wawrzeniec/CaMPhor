...
registers each y-slice of each time frame to the corresponding y-slice of the trial's own baseline
...
class registerYSlicesToBaseline(camphorRegistrationMethod):
    def __init__(self):
        super(registerYSlicesToBaseline, self).__init__()
        self._parameters = registerYSlicesToBaselineParameters()
        self.percentDone = 0
        self.nDone = 0
        self.nTotal = 0
...
class registerYSlicesToBaselineParameters(object):
    def __init__(self):
        self.learningRate = 1
...
class registerYSlicesToBaselineTransform(transform.transform):
    ...
    def apply(self, data):
        ...
        print("Applying registerYSlicesToBaselineTransform")
        nslices = data[0].shape[2]
...
filter = registerYSlicesToBaseline
